...
export type TaggedModelIdentity={name:string;digest:string;tagParameters:string|null;capabilities:string[]};
export type ModelNames={generation:string;supportedGeneration:string;embedding:string};
export type ModelTag={name:string;digest:string};
export type ModelIdentity=Record<keyof ModelNames,TaggedModelIdentity>&{ollamaVersion:string};
...
/** Read metadata only; never substitute the historical model for a missing candidate. */
export async function identifyTaggedModels(names:ModelNames,tags:ModelTag[],show:(name:string)=>Promise<{parameters?:unknown;capabilities?:unknown}>):Promise<Record<keyof ModelNames,TaggedModelIdentity>>{
 const byName=new Map<string,ModelTag>();
 for(const [role,name] of Object.entries(names)){
  if(typeof name!=='string'||!name.trim())throw Error(`No model name was exported for ${role}.`);
  const tag=tags.find(model=>model.name===name);
  if(!tag?.digest)throw Error(`Required ${role} model is not installed or has no digest: ${name}.`);
  byName.set(name,tag);
 }
 const details=new Map(await Promise.all([...byName].map(async([name,tag])=>{
  const metadata=await show(name);
  if(metadata.parameters!==undefined&&typeof metadata.parameters!=='string')throw Error(`Invalid tag parameters returned for ${name}.`);
  if(!Array.isArray(metadata.capabilities)||metadata.capabilities.some(value=>typeof value!=='string'))throw Error(`Invalid model capabilities returned for ${name}.`);
  // Do not retain modelfile, FROM paths, system text, templates or other show fields.
  return[name,{name,digest:tag.digest,tagParameters:metadata.parameters??null,capabilities:[...metadata.capabilities].sort()} satisfies TaggedModelIdentity] as const;
 })));
 return Object.fromEntries(Object.entries(names).map(([role,name])=>[role,details.get(name)!])) as Record<keyof ModelNames,TaggedModelIdentity>;
}

export function validateResumeConfiguration(manifest:{configurationHash?:unknown;configuration?:unknown}|undefined,configuration:unknown):void{
 if(manifest&&(manifest.configurationHash!==objectHash(configuration)||canonicalJson(manifest.configuration)!==canonicalJson(configuration)))throw Error('Output directory belongs to a different configuration. Use a new directory; existing results remain unchanged.');
}
