...
const draftSchema=z.discriminatedUnion('insufficient',[
 z.object({basis:z.string().min(1).max(400),insufficient:z.literal(false),missing:z.literal(''),claims:z.array(claimSchema).min(1).max(3)}).strict(),
 z.object({basis:z.string().min(1).max(400),insufficient:z.literal(true),missing:z.string().trim().min(1).max(600),claims:z.array(claimSchema).length(0)}).strict()
]);
...
export function createSupportedAnswer(dependencies:Dependencies){
 return async function supportedAnswer(course:Course,question:string,language='en',callerSignal?:AbortSignal){
  ...
  try{
   ...
   return {answer:outcome==='supported'?claims.map(c=>c.text).join('\n\n'):refusal(outcome,language),insufficient:outcome!=='supported',citations,mode:'support-reviewed',model,elapsedMs:Math.round(performance.now()-started),sources,attempts,support:{policy:SUPPORT_POLICY,outcome,disclosure:supportDisclosure,claims,review:lastReview??null,modelCalls}};
  }catch(error){
