...
export const SUPPORT_POLICY='progressive-source-conclusion-review-v4';
export const REQUEST_TIMEOUT_MS=180_000;
export const SUPPORT_OPTIONS=OPTIONS;
export const SUPPORT_THINK=THINK;
export const SUPPORT_SOURCE_WINDOWS=Object.freeze({firstDraft:3,review:5,retryDraft:5});
export const SUPPORT_CONFIG={think:THINK,truncate:false,options:OPTIONS,sourceWindows:SUPPORT_SOURCE_WINDOWS};
...
export function createSupportedAnswer(dependencies:Dependencies){
 return async function supportedAnswer(course:Course,question:string,language='en',callerSignal?:AbortSignal){
  ...
  try{
   checkCancellation();const selected=(await dependencies.retrieve(course,question,signal)).slice(0,SUPPORT_SOURCE_WINDOWS.review);checkCancellation();
   ...
   const draftEvidence:Array<{attempt:number;sectionIds:string[];excerptIds:string[]}>=[];
   ...
   for(let attempt=1;attempt<=2;attempt++){
    ...
    const draftSourceIds=new Set(selected.slice(0,attempt===1?SUPPORT_SOURCE_WINDOWS.firstDraft:SUPPORT_SOURCE_WINDOWS.retryDraft).map(source=>source.id));
    const narrowedChoices=choices.filter(choice=>draftSourceIds.has(choice.sectionId));
    const allowedChoices=narrowedChoices.length?narrowedChoices:choices;
    const evidenceInput=allowedChoices.map(({id,sectionId,quote})=>({id,sectionId,quote}));
    draftEvidence.push({attempt,sectionIds:[...new Set(allowedChoices.map(choice=>choice.sectionId))],excerptIds:allowedChoices.map(choice=>choice.id)});
    try{
     const input=inputDraftSchema.parse(await call('draft',attempt,draftPrompt(language),{question,language:language==='zh'?'Simplified Chinese':'English',evidence:evidenceInput,previousReview:feedback??null},draftFormatFor(allowedChoices.map(choice=>choice.id))));
     const draft=validateDraft({...input,claims:input.claims.map(claim=>({text:claim.text,citations:resolveEvidenceIds(claim.evidenceIds,allowedChoices)}))},selected,question);
     lastDraft=draft;
     // A narrower first window cannot justify a terminal source-insufficiency claim.
     // Expand before the review, keeping the total at two drafts plus one review.
     if(attempt===1&&draft.insufficient&&allowedChoices.length<choices.length){feedback={evidenceWindowExpanded:true,instruction:'Reconsider the question against the complete supplied evidence window.'};continue;}
     stage='review';
   ...
   return {answer:outcome==='supported'?claims.map(c=>c.text).join('\n\n'):refusal(outcome,language),insufficient:outcome!=='supported',citations,mode:'support-reviewed',model,elapsedMs:Math.round(performance.now()-started),sources,attempts,support:{policy:SUPPORT_POLICY,outcome,disclosure:supportDisclosure,claims,quoteChoices:choices.map(({id,sectionId,start,end})=>({id,sectionId,start,end})),review:lastReview??null,modelCalls,draftEvidence}};
  }catch(error){
