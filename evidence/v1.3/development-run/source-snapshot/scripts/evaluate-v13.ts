...
import {canonicalJson,fileHash,gateCaseRead,objectHash,outputDiagnostics,readLockedJson,resumeKey,sourceGraph,validateDataset,type DatasetLock,type EvaluationCase,type FrozenIdentity,type Mode,type SourceDocument,type Split} from './evaluate-v13-core.ts';
...
async function modelIdentity(ai:any){
 const status=await ai.modelStatus();
 const generation=status.models?.find((m:any)=>m.name===ai.MODEL),embedding=status.models?.find((m:any)=>m.name===ai.EMBED);
 if(!status.ready||!generation?.digest||!embedding?.digest)throw Error('Both frozen model tags and their actual digests must be available before evaluation.');
 ...
 return{generation:{name:ai.MODEL,digest:generation.digest},embedding:{name:ai.EMBED,digest:embedding.digest},ollamaVersion:version.version};
}
...
async function main(){
 ...
 const identity:FrozenIdentity={schemaVersion:1,datasetLockSha256:fileHash(lockPath),runnerSha256:importedSources.runner.hash,
  ...
  model:await modelIdentity(ai),options:{'frozen-grounded':{think:false,options:ai.options},supported:supportConfig},deadlineMs:args.deadlineMs,
 ...
 if(old&&(old.configurationHash!==configurationHash||canonicalJson(old.configuration)!==canonicalJson(config)))throw Error('Output directory belongs to a different configuration. Use a new directory; existing results remain unchanged.');
