...
export async function importDocument(name:string,buffer:Buffer):Promise<Course>{
 ...
 else if(ext==='md'||ext==='txt'){const text=buffer.toString('utf8');if(text.includes('\u0000')||text.includes('\uFFFD'))throw new AppError('Use a UTF-8 text file.');for(let i=0;i<text.length;i+=3500)pages.push(text.slice(i,i+3500));if(pages.length>50)throw new AppError('Text is too long (maximum 175,000 characters).');}
 ...
 const sections:Section[]=[];for(let p=0;p<pages.length;p++){const chunks:string[]=[];let rest=pages[p];while(rest.length){let end=Math.min(1400,rest.length);if(end<rest.length){const boundary=rest.lastIndexOf(' ',end);if(boundary>700)end=boundary+1;}chunks.push(rest.slice(0,end));rest=rest.slice(end);}for(const chunk of chunks){const text=chunk.trim();if(text)sections.push({id:`p${p+1}-${sections.length+1}`,title:text.split('\n')[0].replace(/^#+\s*/,'').slice(0,75),page:p+1,text});}}
