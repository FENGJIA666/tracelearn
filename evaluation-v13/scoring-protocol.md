# Frozen semantic-review protocol, version 1
...
The primary comparison is **the previous production configuration versus the proposed production configuration**, using the same model weights and available fictional materials. Thinking mode, generation/review prompts, token budgets, context selection, and number of internal model calls may differ. Record the exact settings for both configurations at freeze time and include all internal work in latency. This is a comparison of complete configurations, not an isolated ablation of citation review; observed differences cannot all be attributed to one verification step.
